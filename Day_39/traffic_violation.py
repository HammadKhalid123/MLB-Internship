...
class TrafficMonitor:
    def __init__(self, model_path="yolov8n.pt", confidence=0.4, iou=0.5, allowed_direction="RIGHT",
                 restricted_zone=None, zone_type="Polygon", frame_width=640, frame_height=480,
                 movement_threshold=4, history_length=20):
        # ---- Model load wrapped in try/except so a bad path / missing weights
        #      gives a clear error instead of an unhandled crash ----
        try:
            self.model = YOLO(model_path)
        except Exception as e:
            raise RuntimeError(f"Could not load YOLO model '{model_path}': {e}")

        self.confidence = confidence
        self.iou = iou  # NMS overlap threshold: lower = stricter duplicate box removal
    ...
    def process_frame(self, frame):
        """Run detection + tracking on a single frame. Returns (frame, detections).
        Never raises - a bad/corrupt frame just returns no detections so a whole
        video isn't killed by one broken frame."""
        self.frame_index += 1
        try:
        ...
        try:
            results = self.model.track(
                ...
                conf=self.confidence,   # confidence threshold: filters weak detections
                iou=self.iou,           # IoU/NMS threshold: filters duplicate overlapping boxes
                verbose=False
            )
        except Exception:
            return frame, []
        ...
        if results[0].boxes.id is not None:
            ...
            for box, cls, tid, score in zip(boxes, classes, ids, scores):
                ...
                # --- Direction check (real-time, recalculated every frame) ---
...
def render_wrong_way_variant(frame, detections, monitor):
    ...
    for det in detections:
        ...
        # Confidence score now shown next to the class label
        cv2.putText(frame, f"ID:{det['id']} {det['class']} {det['score']:.2f}", (x1, y1 - 10),
...
def render_dashboard_variant(frame, detections, monitor):
    ...
    for det in detections:
        ...
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        cv2.putText(frame, f"ID:{det['id']} {det['score']:.2f}", (x1, y1 - 8),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    return frame